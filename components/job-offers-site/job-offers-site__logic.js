...
const Logic = ({ render, jobOffers }) => {
  ...
  const handleSearchInput = value => {
    ...
    const filterByLocation = jobOffers.filter(offer =>
      offer.city.toUpperCase().includes(locationValue.toUpperCase())
    );

    if (value.length > 0) {
      const filtereddOffers = filterByLocation.filter(offer =>
        offer.title.toUpperCase().includes(value.toUpperCase())
      );
  ...
  const handleLocationInput = value => {
    ...
    const filterBySearch = jobOffers.filter(offer =>
      offer.title.toUpperCase().includes(searchValue.toUpperCase())
    );

    if (value.length > 0) {
      const filtereddOffers = filterBySearch.filter(offer =>
        offer.city.toUpperCase().includes(value.toUpperCase())
      );
      sortOffers(filtereddOffers);
