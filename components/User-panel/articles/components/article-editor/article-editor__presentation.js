      ...
      <ContentContainer>
        ...
        {state.pictrue ? (
          <FileUploader onDrop={handleDropFiles} type='succes'>
            ...
            <p>
              {state.pictrue.name.slice(0, 10) +
                '...' +
                state.pictrue.name.slice(state.pictrue.name.length - 3, state.pictrue.name.length)}
            </p>
          </FileUploader>
        ...
        {state.sections &&
          state.sections.map((section, index) => {
            if (section.type === 'acapit') {
              return (
                <NewSection key={index}>
                  <ActionButtonsContainer>
                    <DeleteButton color={grey} onClick={() => handleDeleteSectionButton(index)} />
                  </ActionButtonsContainer>
            ...
            } else if (section.type === 'edit')
              return <EditInformation key={index}>{section.value}</EditInformation>;
          })}

        {/* ACTION SECTIONS BUTTON */}
        <SectionButtonsContainer>
          {sectionTypes &&
            sectionTypes.map((button, index) => (
              <SectionButton key={index} icon={button.icon} onClick={() => handleAddNewSection(button.type)}>
                {button.name}
              </SectionButton>
            ))}
        </SectionButtonsContainer>
        ...
        <ActionButtonsContainer>
          ...
          {state.isLoading ? (
            <Button>
              <Loading text={language.articlesPanel.articleEditor.loading} />
            </Button>
          ) : editMode ? (
