...
import { Container, ChoseFieldContainer } from './add-coop-offer-window__styles';
...
const AddCooperationOfferWindow = ({ close, handleInputs, inputs, submit }) => {
  const language = useSelector(s => s.language.source.companyPanel.addNewCooperationOfferWindow);
    ...
    <Window width='50vw' close={() => close(true)}>
      <Container>
        ...
        {inputsArray.map((input, i) => (
          <React.Fragment>
            <Input
              ...
              label={input.label}
              value={input.value}
              onChange={e => handleInputs({ inputName: input.name, value: e.target.value })}
              name={input.name}
              type={input.type}
            />
            <Separator height='20px' />
        ...
        {inputs.isLoading ? (
          ...
          <Button onClickFunction={() => submit(inputs.isEditMode)}>
            {inputs.isEditMode ? language.edit : language.submit}
          </Button>
        )}
      </Container>
    </Window>
