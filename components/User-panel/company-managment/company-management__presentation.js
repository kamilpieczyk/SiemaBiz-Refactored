    ...
    <Container>
      {
        ...
        state.addNewCompanyWindow.isActive && (
          <AddCompanyWindow
            close={() => handlers.handleAddNewCompanyButton(true)}
            edit={state.addNewCompanyWindow.editID}
          />
        )
      }
      {
              ...
              <section>
                {state.employeesWindow.owners.map(owner => (
                  <EmployeeBox key={owner}>
                    <Link href={{ pathname: '/user', query: { username: owner } }}>
                      <a>{owner}</a>
                    </Link>
                    <MaterialIcon
                      icon='remove_circle'
...

      {
        ...
        state.addJobAd.isActive && (
          <Window width='80%' close={() => handlers.handleJobAdWindow(false, true, {})}>
            <AddJobAdWindow>
              <h1>
                {state.addJobAd.companyName} - {language.addNewJobAd}
              </h1>
              <section>
              ...
              <ClickButton
                maxWidth
                onClickFunction={() =>
                  handlers.handleJobAdWindowSubmit({
...

      {
        ...
        state.cvWindow.isActive && (
          <CvWindow close={handlers.handleCvWindow} applications={state.cvWindow.applications} />
        )
      }
              ...
              state.jobAdsWindow.isActive && company._id === state.jobAdsWindow.companyID && (
                <CompanieContentContainer>
                  ...
                  <ClickButton
                    maxWidth
                    onClickFunction={() =>
                      handlers.handleJobAdWindow(false, false, {
                  ...
                  {state.jobAdsWindow.jobAds?.map(jobAd => (
                        ...
                        <React.Fragment>
                          ...
                          <section>
                            ...
                            <MaterialIcon
                              ...
                              title={language.edit}
                              onClick={() =>
                                handlers.handleJobAdWindow(jobAd._id, false, {
                                  companyID: company._id,
                                  companyName: company.name,
                                })
                              }
...

                  {
                    ...
                    state.jobAdsWindow.jobAds.length === 0 && <Warning>{language.nothingToShow}</Warning>
                  }
                ...
                <CompanieContentContainer>
                  ...
                  {state.cooperationOffers.cooperationOffers?.map(offer => (
                    <JobAdBox key={offer._id}>
                      {state.isArchiviseActive.bool && offer._id === state.isArchiviseActive.id ? (
                        <>
                        ...
                        <>
                          ...
                          <section>
                            ...
                            <MaterialIcon
                              icon='archive'
                              title={language.archivise}
                              onClick={() => handlers.handleArchiviseCooperationOffer(offer._id)}
                            />
                          </section>
                        </>
                      )}
...
CompanyManagementPresentation.propTypes = {
  ...
  handlers: PropTypes.shape({
    ...
    handleArchiviseCooperationOffer: PropTypes.func.isRequired,
  }),
};
