...
const MyCvPresentationLayer = ({
  ...
  return (
    <Container id='my-cv'>
      <Breadcrumbs generalCrumb={language.userPanel.myCv.title} breadcrumbs={breadcrumbs} />

      <MainDetails>
        <Separator height='30px' />
        {mainInformationInputs.map((element, index) => (
          <React.Fragment key={index}>
            <Input key={index} value={element.value} onChange={element.onChange} label={element.label} />
      ...
      <Section>
        ...
        <Separator height='20px' />
        {state.workplaces &&
          state.workplaces.map((workplace, i) => (
            <React.Fragment key={i}>
              <SectionInSection>
                <Input
      ...
      <Section>
        <Separator height='20px' />
        <TitleOfSection>{language.userPanel.myCv.hobbies.title}</TitleOfSection>
        {state.hobbies &&
